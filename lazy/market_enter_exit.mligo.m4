...
let enter_exit_market ( args, business_storage : enter_exit_args * business_storage ) : operation list * business_storage =
	...
	let trade : market_trade_params = { market_id = market_id; amount = args.amount; } in
	let ( external_token_operation, bootstrapped_market_data, token_storage ) = match args.direction with
	| Mint -> (
		let pull_payment = get_pull_payment ( market_data.metadata.currency, args.amount ) in
		let token_storage = mint_prediction_tokens ( Tezos.sender, trade, token_storage ) in
		let currency_pool_delta = split_revenue args.amount in
	...
	| Burn -> (
		let token_storage = burn_prediction_tokens ( Tezos.sender, trade, token_storage ) in
		let currency_payout_data = calculate_currency_payout ( trade, bootstrapped_market_data.currency_pool.market_currency_pool, token_storage ) in
